...
var BaseConfig = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      defaultEngine: import_koishi.Schema.const("BERT-VITS2").required(),
    ...
    import_koishi.Schema.object({
      defaultEngine: import_koishi.Schema.const("GPT-SOVITS").required(),
...
${speaker_text}`;
  ...
  async baseSay(option, engine) {
    ...
    try {
      ...
      formData.append("reference_audio", (0, import_fs.createReadStream)(this.baseConfig.reference_audio), {
        filename: import_path.default.basename(this.baseConfig.reference_audio),
        // ... or:
        contentType: "audio/wav"
      });
      let url = `${this.http.config.baseURL}/voice${ENGINE_MAP[engine]}`;
      const response = await import_axios.default.post(url, formData, { responseType: "arraybuffer" });
      return import_koishi2.h.audio(response.data, "audio/mpeg");
    } catch (e) {
      logger.info(e);
